...
def topic_flow_download(request, court, topic, role, output_id):
    ...
    ``output_type`` to assemble the file from the guest's stored answers. The
    view stays thin — file bytes come from the download handlers in
    downloads.py, computed from the same AnswerStore the page renders, so the
