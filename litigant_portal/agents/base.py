...
class Agent:
    ...
    Example:
        class LegalAssistant(Agent):
            ...
            tool_classes = [SearchDocuments, GetCourtInfo]
    ...
    def __init__(
        ...
        **completion_args: Any,
        ...
        Args:
            thread: ChatThread object to associate with the agent.
        """
        self.thread = thread
        self.completion_args = completion_args

        # Set up tools and messages
        self.tools = {tool.__name__: tool for tool in self.tool_classes or []}
        self.tool_schemas = [
            tool.get_schema() for tool in self.tool_classes or []
        ]
        self.messages: list[Message] = self.thread.history
    ...
    def add_message(self, message: Message):
        ...
        from litigant_portal.app.models import ChatMessage

        self.messages.append(message)
        ChatMessage.objects.create(thread=self.thread, data=message)
        ...
        **completion_args: Any,
        ...
        try:
            for _ in range(self.max_steps):
                ...
                call_args.setdefault("model", "openai/gpt-5-mini")
                call_args.setdefault("drop_params", True)
