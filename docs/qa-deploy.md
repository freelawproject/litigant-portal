...
- **`reset_db`** — default **true**: the deploy drops every table and rebuilds the database from the deployed ref (#881). Untick it to keep the current QA data across the deploy.
...
4. **Temp pod** — a throwaway pod (`temp-pod-<sha>`, label `app=litigant-deploy-temp`) starts on the new image with the real secrets. All state changes run here, _before_ the web pods are touched:
   ...
   - `migrate --noinput`
   - with `reset_db`: `sync_corpus --strict` (corpus YAML → database rows) and `bootstrap_superuser`, then scale `litigant-web` back up
